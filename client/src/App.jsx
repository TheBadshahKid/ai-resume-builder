...
import {
  ...
  BarChart2, Target, Download, ChevronRight, Star, Users, Award
} from 'lucide-react';
...
function Home() {
  return (
    <div className="bg-[#0f0f11] text-white min-h-screen overflow-x-hidden">

      {/* ── HERO ── */}
      <section className="relative flex flex-col items-center justify-center text-center px-4 pt-28 pb-24 overflow-hidden">
        {/* Glow blobs */}
        <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-brand-rust opacity-[0.12] rounded-full blur-[120px] pointer-events-none" />
        <div className="absolute bottom-0 right-1/4 w-[400px] h-[400px] bg-indigo-600 opacity-[0.10] rounded-full blur-[120px] pointer-events-none" />

        {/* Badge */}
        <div className="inline-flex items-center gap-2 bg-white/5 border border-white/10 text-orange-400 text-xs font-semibold px-4 py-1.5 rounded-full mb-6 backdrop-blur-sm">
          <Sparkles size={12} /> Powered by Llama 3 &amp; Groq — Free AI
        </div>

        <h1 className="text-5xl md:text-7xl font-black tracking-tight leading-[1.05] max-w-4xl mb-6 font-display">
          Land Your Dream Job
          <br />
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center">
          ...
          <Link
            to="/login"
            className="inline-flex items-center gap-2 text-gray-400 hover:text-white px-6 py-4 rounded-2xl font-medium border border-white/10 hover:border-white/20 transition-all text-sm"
          >
      ...
      <section className="border-y border-white/5 bg-white/[0.02] py-12 px-4">
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6">
          {stats.map((s) => (
            <div key={s.label} className="flex flex-col items-center text-center gap-2 p-6 rounded-2xl bg-white/[0.03] border border-white/5 hover:border-white/10 transition-all">
              <div className="text-brand-rust">{s.icon}</div>
              <div className="text-3xl font-black text-white font-display">{s.value}</div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <h2 className="text-4xl md:text-5xl font-black text-white font-display mb-4">
              ...
              <br />
              <span className="text-gray-500">Supercharged by AI</span>
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((f) => (
              <div
                key={f.title}
                className="group relative rounded-2xl p-6 bg-white/[0.03] border border-white/[0.07] hover:border-white/15 hover:bg-white/[0.05] transition-all duration-300 overflow-hidden"
              >
                {/* Icon glow blob */}
                <div className={`absolute -top-6 -right-6 w-24 h-24 bg-gradient-to-br ${f.color} opacity-10 rounded-full blur-2xl group-hover:opacity-20 transition-opacity`} />
                ...
                <h3 className="text-white font-bold text-base mb-2 font-display">{f.title}</h3>
      ...
      <section className="py-20 px-4">
        <div className="max-w-3xl mx-auto text-center rounded-3xl border border-white/10 bg-gradient-to-br from-brand-rust/10 to-transparent p-14 relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-brand-rust/5 to-indigo-600/5 rounded-3xl" />
          <h2 className="text-3xl md:text-4xl font-black text-white mb-4 relative font-display">
            Ready to Beat the Bots?
          </h2>
          <p className="text-gray-400 mb-8 relative">Start building your ATS-optimized resume right now — no account needed.</p>
          <Link
            to="/builder"
      ...
      <footer className="border-t border-white/5 py-8 px-4 text-center text-gray-600 text-xs">
        <div className="flex items-center justify-center gap-2 mb-2">
          ...
          <span className="font-bold text-gray-400">ResumeAI</span>
        </div>
        Built with 💙 · AI by Groq (Llama 3) · © {new Date().getFullYear()}
      </footer>
    </div>
  );
}
...
function Navigation() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-6 md:px-10 py-4 border-b border-white/5 bg-[#0f0f11]/80 backdrop-blur-xl">
        ...
        {isAuthenticated ? (
          <>
            ...
            <button
              ...
              className="text-sm font-medium text-gray-400 hover:text-white border border-white/10 hover:border-white/20 px-4 py-2 rounded-xl transition-all"
            >
              Log out
            </button>
          </>
...
function AppContent() {
  return (
    <div className="min-h-screen font-sans bg-[#0f0f11]">
      <Navigation />
